...
RUN mkdir -p /usr/src/geonode_wrapper
...
COPY src /usr/src/geonode_wrapper/
WORKDIR /usr/src/geonode_wrapper

COPY src/wait-for-databases.sh /usr/bin/wait-for-databases
RUN chmod +x /usr/bin/wait-for-databases
RUN chmod +x /usr/src/geonode_wrapper/tasks.py \
    && chmod +x /usr/src/geonode_wrapper/entrypoint.sh
